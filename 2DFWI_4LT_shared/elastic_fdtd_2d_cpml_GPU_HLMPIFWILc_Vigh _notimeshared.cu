...
#define Lpo 2	//S_LC_space
#define Lso 4	//P_LC_space
		...
		)
{
	...
	__shared__ float s_Gp[BLOCK_SIZE][BLOCK_SIZE][Lpo+1],s_Gs[BLOCK_SIZE][BLOCK_SIZE][Lso+1];
	...
	for(ic=0;ic<Lp;ic++)
		s_Gp[ty][tx][ic]=Gp[index1*Lp+ic];
	for(ic=0;ic<Ls;ic++)
		s_Gs[ty][tx][ic]=Gs[index2*Ls+ic];
	...
	if(iz>=Lb&&iz<=ntz-Lb&&ix>=Lb-1&&ix<=ntx-Lb-1)
	{
		...
		for(ic=0;ic<Lpo;ic++)
			dsigmaxx_dx+=s_Gp[ty][tx][ic]*(s_sigmaxx[ty][s_txp+ic+1]-s_sigmaxx[ty][s_txp-ic])*one_over_dx;
		
		dsigmaxx_dx+=s_Gp[ty][tx][Lpo]*(sigmaxx[ip+ntx+1]-sigmaxx[ip+ntx]+sigmaxx[ip-ntx+1]-sigmaxx[ip-ntx])*one_over_dx;	//x direction

		for(ic=0;ic<Lso;ic++)
		{
			dsigmaxxs_dx+=s_Gs[ty][tx][ic]*(s_sigmaxxs[ty][s_txs+ic+1]-s_sigmaxxs[ty][s_txs-ic])*one_over_dx;
			dsigmaxz_dz+=s_Gs[ty][tx][ic]*(s_sigmaxz[s_tys+ic][tx]-s_sigmaxz[s_tys-(ic+1)][tx])*one_over_dz;
		}

		dsigmaxxs_dx+=s_Gs[ty][tx][Lso]*(sigmaxxs[ip+ntx+1]-sigmaxxs[ip+ntx]+sigmaxxs[ip-ntx+1]-sigmaxxs[ip-ntx])*one_over_dx;	//x direction
		dsigmaxz_dz+=s_Gs[ty][tx][Lso]*(sigmaxz[ip+1]-sigmaxz[ip-ntx+1]+sigmaxz[ip-1]-sigmaxz[ip-ntx-1])*one_over_dz;
		...
		)
{
	...
	if(iz>=Lb-1&&iz<=ntz-Lb-1&&ix>=Lb&&ix<=ntx-Lb)
	{
		...
		dsigmaxx_dz+=Gp[index1*Lp+Lpo]*(sigmaxx[ip+ntx+1]-sigmaxx[ip+1]+sigmaxx[ip+ntx-1]-sigmaxx[ip-ntx-1])*one_over_dz;
		...
		dsigmazz_dz+=Gs[index2*Ls+Lso]*(sigmazz[ip+ntx+1]-sigmazz[ip+1]+sigmazz[ip+ntx-1]-sigmazz[ip-ntx-1])*one_over_dz;
		...
		)
{
	...
	if(iz>=Lb-1&&iz<=ntz-Lb-1&&ix>=Lb&&ix<=ntx-Lb)
	{
		...
		for(ic=0;ic<Lpo;ic++)
		{
			dsigmaxx_dz+=s_Gp[ty][tx][ic]*(s_sigmaxx[s_typ+(ic+1)][tx]-s_sigmaxx[s_typ-ic][tx])*one_over_dz;
		}
		dsigmaxx_dz+=s_Gp[ty][tx][Lpo]*(sigmaxx[ip+ntx+1]-sigmaxx[ip+1]+sigmaxx[ip+ntx-1]-sigmaxx[ip-ntx-1])*one_over_dz;


		for(ic=0;ic<Lso;ic++)
		{
			dsigmaxz_dx+=s_Gs[ty][tx][ic]*(s_sigmaxz[ty][s_txs+ic]-s_sigmaxz[ty][s_txs-(ic+1)])*one_over_dx;
			dsigmazz_dz+=s_Gs[ty][tx][ic]*(s_sigmazz[s_tys+(ic+1)][tx]-s_sigmazz[s_tys-ic][tx])*one_over_dz;
		}
		dsigmaxz_dx+=s_Gs[ty][tx][Lso]*(sigmaxz[ip+ntx]-sigmaxz[ip+ntx-1]+sigmaxz[ip-ntx]-sigmaxz[ip-ntx-1])*one_over_dx;
		dsigmazz_dz+=s_Gs[ty][tx][Lso]*(sigmazz[ip+ntx+1]-sigmazz[ip+1]+sigmazz[ip+ntx-1]-sigmazz[ip-ntx-1])*one_over_dz;
...

{
	...
	if(iz>=Lb&&iz<=ntz-Lb&&ix>=Lb&&ix<=ntx-Lb)
	{
		...
		for(ic=0;ic<Lpo;ic++)
		{
			dvx_dx+=s_Gp[ty][tx][ic]*(s_vxp[ty][s_txp+ic]-s_vxp[ty][s_txp-(ic+1)])*one_over_dx;
			dvz_dz+=s_Gp[ty][tx][ic]*(s_vzp[s_typ+ic][tx]-s_vzp[s_typ-(ic+1)][tx])*one_over_dz;
		}
		dvx_dx+=s_Gp[ty][tx][Lpo]*(vx[ip+ntx]-vx[ip+ntx-1]+vx[ip-ntx]-vx[ip-ntx-1])*one_over_dx;
		dvz_dz+=s_Gp[ty][tx][Lpo]*(vz[ip+1]-vz[ip-ntx+1]+vz[ip-1]-vz[ip-ntx-1])*one_over_dz;
		...
		for(ic=0;ic<Lso;ic++)
		{
			dvx_dx+=s_Gs[ty][tx][ic]*(s_vxs[ty][s_txs+ic]-s_vxs[ty][s_txs-(ic+1)])*one_over_dx;
			dvz_dz+=s_Gs[ty][tx][ic]*(s_vzs[s_tys+ic][tx]-s_vzs[s_tys-(ic+1)][tx])*one_over_dz;
		}
		dvx_dx+=s_Gs[ty][tx][Lso]*(vx[ip+ntx]-vx[ip+ntx-1]+vx[ip-ntx]-vx[ip-ntx-1])*one_over_dx;
		dvz_dz+=s_Gs[ty][tx][Lso]*(vz[ip+1]-vz[ip-ntx+1]+vz[ip-1]-vz[ip-ntx-1])*one_over_dz;
		...
		)
{
	...
	__shared__ float s_Gs[BLOCK_SIZE][BLOCK_SIZE][Lso+1];
	...
	if(iz>=Lb-1&&iz<=ntz-Lb-1&&ix>=Lb-1&&ix<=ntx-Lb-1)
	{
		...
		for(ic=0;ic<Lso;ic++)
		{
			dvz_dx+=s_Gs[ty][tx][ic]*(s_vz[ty][s_txs+ic+1]-s_vz[ty][s_txs-ic])*one_over_dx;
			dvx_dz+=s_Gs[ty][tx][ic]*(s_vx[s_tys+(ic+1)][tx]-s_vx[s_tys-ic][tx])*one_over_dz;
		}

		dvz_dx+=s_Gs[ty][tx][Lso]*(vz[ip+1+ntx]-vz[ip+ntx]+vz[ip+1-ntx]-vz[ip-ntx])*one_over_dx;
		dvx_dz+=s_Gs[ty][tx][Lso]*(vx[ip+ntx+1]-vx[ip+1]+vx[ip+ntx-1]-vx[ip-1])*one_over_dz;
 ...
 )
{
	...
	if(iz>=pmlc+1&&iz<=ntz-pmlc-1&&ix>=pmlc+1&&ix<=ntx-pmlc-1)
	{
		...
		image_vp[ip]=image_vp[ip]+(2*lambda[ip]+2*mu[ip])*(dvx_dx+dvz_dz)*(sigmaxx[ip]+sigmazz[ip]);
		image_vs[ip]=image_vs[ip]+(lambda_plus_two_mu[ip]*dvx_dx+lambda[ip]*dvz_dz)*sigmaxxs[ip]
			+(lambda_plus_two_mu[ip]*dvz_dz+lambda[ip]*dvx_dx)*sigmazz[ip]
...
*/
	...
	for(it=itmax-2;it>=0;it--)
	{
		...
		/*	fdtd_cpml_2d_GPU_kernel_vx_mine_backward<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rho, plan[i].d_lambda, plan[i].d_mu, plan[i].d_lambda_plus_two_mu,
				 plan[i].d_a_x_half, plan[i].d_a_z, 
				 plan[i].d_b_x_half, plan[i].d_b_z, 
				 plan[i].d_vx, plan[i].d_sigmaxx, plan[i].d_sigmazz, plan[i].d_sigmaxz,
				 plan[i].d_phi_sigmaxx_x, plan[i].d_phi_sigmazz_x, plan[i].d_phi_sigmaxz_z, 
				 ntp, ntx, ntz, 
				 -dx, -dz, dt,
				 plan[i].d_seismogram_vx_rms, it, pml, Lc, plan[i].d_rc, ss[is+i].r_iz, plan[i].d_r_ix, ss[is+i].r_n
				);

			fdtd_cpml_2d_GPU_kernel_vz_mine_backward<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rho, plan[i].d_lambda, plan[i].d_mu, plan[i].d_lambda_plus_two_mu,
				 plan[i].d_a_x, plan[i].d_a_z_half,
				 plan[i].d_b_x, plan[i].d_b_z_half,
				 plan[i].d_vz, plan[i].d_sigmaxx, plan[i].d_sigmazz, plan[i].d_sigmaxz, 
				 plan[i].d_phi_sigmaxx_z, plan[i].d_phi_sigmazz_z, plan[i].d_phi_sigmaxz_x,
				 ntp, ntx, ntz, -dx, -dz, dt,
				 plan[i].d_seismogram_vz_rms, it, pml, Lc, plan[i].d_rc, ss[is+i].r_iz, plan[i].d_r_ix, ss[is+i].r_n
				);

			fdtd_cpml_2d_GPU_kernel_sigmaxx_sigmazz_mine_backward<<<dimGrid,dimBlock,0,plans[i].stream>>>
				( 
				 plan[i].d_a_x, plan[i].d_a_z,
				 plan[i].d_b_x, plan[i].d_b_z,
				 plan[i].d_vx, plan[i].d_vz, plan[i].d_sigmaxx, plan[i].d_sigmazz,
				 plan[i].d_phi_vx_x, plan[i].d_phi_vz_z, 
				 ntp, ntx, ntz, -dx, -dz, dt, Lc, plan[i].d_rc
				);

			fdtd_cpml_2d_GPU_kernel_sigmaxz_mine_backward<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_a_x_half, plan[i].d_a_z_half,
				 plan[i].d_b_x_half, plan[i].d_b_z_half,
				 plan[i].d_vx, plan[i].d_vz, plan[i].d_sigmaxz,
