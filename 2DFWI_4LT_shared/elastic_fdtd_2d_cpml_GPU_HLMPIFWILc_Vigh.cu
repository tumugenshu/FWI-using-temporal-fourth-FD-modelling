...
__global__ void fdtd_cpml_2d_GPU_kernel_vx_4LT_shared(
		float *rho, int itmax, float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *a_x_half, float *a_z, 
		...
		)
{
	...
	/***start the assignment of the shared memory***/
	__shared__ float s_sigmaxx[BLOCK_SIZE][BLOCK_SIZE+2*Lpo];
	__shared__ float s_sigmaxxs[BLOCK_SIZE][BLOCK_SIZE+2*Lso];
	__shared__ float s_sigmaxz[BLOCK_SIZE+2*Lso][BLOCK_SIZE];
		
	s_sigmaxx[ty][s_txp]=sigmaxx[ip];
	s_sigmaxxs[ty][s_txs]=sigmaxxs[ip];
	s_sigmaxz[s_tys][tx]=sigmaxz[ip];	
		
	// in x_index_sigmaxx LPO
	if(tx<Lpo)
		if(bx)
			s_sigmaxx[ty][tx]=sigmaxx[ip-Lpo];
		else
			s_sigmaxx[ty][tx]=0.0;
	if(tx>blockDim.x-Lpo-1)
		if(bx<gridDim.x-1)
			s_sigmaxx[ty][tx+2*Lpo]=sigmaxx[ip+Lpo];
		else
			s_sigmaxx[ty][tx+2*Lpo]=0.0;

	// in x_index_sigmaxxs LSO	
	if(tx<Lso)
		if(bx)
			s_sigmaxxs[ty][tx]=sigmaxxs[ip-Lso];
		else
			s_sigmaxxs[ty][tx]=0.0;
	if(tx>blockDim.x-Lso-1)
		if(bx<gridDim.x-1)
			s_sigmaxxs[ty][tx+2*Lso]=sigmaxxs[ip+Lso];
		else
			s_sigmaxxs[ty][tx+2*Lso]=0.0;

	// in y_index_sigmaxz LSO		
	if(ty<Lso)
		if(by)
			s_sigmaxz[ty][tx]=sigmaxz[ip-Lso*ntx];
		else
			s_sigmaxz[ty][tx]=0.0;
	if(ty>blockDim.y-Lso-1)
		if(by<gridDim.y-1)
			s_sigmaxz[ty+2*Lso][tx]=sigmaxz[ip+Lso*ntx];
		else
			s_sigmaxz[ty+2*Lso][tx]=0.0;
	...
	if(iz>=Lb&&iz<=ntz-Lb&&ix>=Lb-1&&ix<=ntx-Lb-1)
	{
		...
		for(ic=0;ic<Lpo;ic++)
			dsigmaxx_dx+=Gp[index1*Lp+ic]*(s_sigmaxx[ty][s_txp+ic+1]-s_sigmaxx[ty][s_txp-ic])*one_over_dx;
		if(tx>0&&tx<BLOCK_SIZE-1&&ty>0&&ty<BLOCK_SIZE-1)
			dsigmaxx_dx+=Gp[index1*Lp+Lpo]*(s_sigmaxx[ty+1][s_txp+1]-s_sigmaxx[ty+1][s_txp]+s_sigmaxx[ty-1][s_txp+1]-s_sigmaxx[ty-1][s_txp])*one_over_dx;
		else dsigmaxx_dx+=Gp[index1*Lp+Lpo]*(sigmaxx[ip+ntx+1]-sigmaxx[ip+ntx]+sigmaxx[ip-ntx+1]-sigmaxx[ip-ntx])*one_over_dx;	//x direction

		for(ic=0;ic<Lso;ic++)
		{
			dsigmaxxs_dx+=Gs[index2*Ls+ic]*(s_sigmaxxs[ty][s_txs+ic+1]-s_sigmaxxs[ty][s_txs-ic])*one_over_dx;
			dsigmaxz_dz+=Gs[index2*Ls+ic]*(s_sigmaxz[s_tys+ic][tx]-s_sigmaxz[s_tys-(ic+1)][tx])*one_over_dz;
		}

		if(tx>0&&tx<BLOCK_SIZE-1&&ty>0&&ty<BLOCK_SIZE-1)
		{
			dsigmaxxs_dx+=Gs[index2*Ls+Lso]*(s_sigmaxxs[ty+1][s_txs+1]-s_sigmaxxs[ty+1][s_txs]+s_sigmaxxs[ty-1][s_txs+1]-s_sigmaxxs[ty-1][s_txs])*one_over_dx;
			dsigmaxz_dz+=Gs[index2*Ls+Lso]*(s_sigmaxz[s_tys][tx+1]-s_sigmaxz[s_tys-1][tx+1]+s_sigmaxz[s_tys][tx-1]-s_sigmaxz[s_tys-1][tx-1])*one_over_dz;
		}
		else{
			dsigmaxxs_dx+=Gs[index2*Ls+Lso]*(sigmaxxs[ip+ntx+1]-sigmaxxs[ip+ntx]+sigmaxxs[ip-ntx+1]-sigmaxxs[ip-ntx])*one_over_dx;	//x direction
			dsigmaxz_dz+=Gs[index2*Ls+Lso]*(sigmaxz[ip+1]-sigmaxz[ip-ntx+1]+sigmaxz[ip-1]-sigmaxz[ip-ntx-1])*one_over_dz;
		...
		)
{
	...
	//start the assignment of the shared memory//
	__shared__ float s_sigmaxx[BLOCK_SIZE+2][BLOCK_SIZE+2*Lpo];
	__shared__ float s_sigmaxxs[BLOCK_SIZE+2][BLOCK_SIZE+2*Lso];
	__shared__ float s_sigmaxz[BLOCK_SIZE+2*Lso][BLOCK_SIZE+2];
		
	s_sigmaxx[tyt][s_txp]=sigmaxx[ip];
	s_sigmaxxs[tyt][s_txs]=sigmaxxs[ip];
	s_sigmaxz[s_tys][txt]=sigmaxz[ip];	
		
	// in x_index_sigmaxx LPO
	if(tx<Lpo)
		if(bx)
			s_sigmaxx[tyt][tx]=sigmaxx[ip-Lpo];
		else
			s_sigmaxx[tyt][tx]=0.0;
	if(tx>blockDim.x-Lpo-1)
		if(bx<gridDim.x-1)
			s_sigmaxx[tyt][tx+2*Lpo]=sigmaxx[ip+Lpo];
		else
			s_sigmaxx[tyt][tx+2*Lpo]=0.0;
	//ylaer
	if(ty<1)
		if(by)
			s_sigmaxx[ty][s_txp]=sigmaxx[ip-ntx];
		else
			s_sigmaxx[ty][s_txp]=0.0;
	if(ty>blockDim.y-1-1)
		if(by<gridDim.y-1)
			s_sigmaxx[ty+2][s_txp]=sigmaxx[ip+ntx];
		else
			s_sigmaxx[ty+2][s_txp]=0.0;

	// in x_index_sigmaxxs LSO	
	if(tx<Lso)
		if(bx)
			s_sigmaxxs[tyt][tx]=sigmaxxs[ip-Lso];
		else
			s_sigmaxxs[tyt][tx]=0.0;
	if(tx>blockDim.x-Lso-1)
		if(bx<gridDim.x-1)
			s_sigmaxxs[tyt][tx+2*Lso]=sigmaxxs[ip+Lso];
		else
			s_sigmaxxs[tyt][tx+2*Lso]=0.0;
	//ylaer
	if(ty<1)
		if(by)
			s_sigmaxxs[ty][s_txs]=sigmaxxs[ip-ntx];
		else
			s_sigmaxxs[ty][s_txs]=0.0;
	if(ty>blockDim.y-1-1)
		if(by<gridDim.y-1)
			s_sigmaxxs[ty+2][s_txs]=sigmaxxs[ip+ntx];
		else
			s_sigmaxxs[ty+2][s_txs]=0.0;

	// in y_index_sigmaxz LSO		
	if(ty<Lso)
		if(by)
			s_sigmaxz[ty][txt]=sigmaxz[ip-Lso*ntx];
		else
			s_sigmaxz[ty][txt]=0.0;
	if(ty>blockDim.y-Lso-1)
		if(by<gridDim.y-1)
			s_sigmaxz[ty+2*Lso][txt]=sigmaxz[ip+Lso*ntx];
		else
			s_sigmaxz[ty+2*Lso][txt]=0.0;
	//xlayer		
	if(tx<1)
		if(bx)
			s_sigmaxz[s_tys][tx]=sigmaxz[ip-1];
		else
			s_sigmaxz[s_tys][tx]=0.0;
	if(tx>blockDim.x-1-1)
		if(by<gridDim.x-1)
			s_sigmaxz[s_tys][tx+2]=sigmaxz[ip+1];
		else
			s_sigmaxz[s_tys][tx+2]=0.0;
	...
	if(iz>=Lb&&iz<=ntz-Lb&&ix>=Lb-1&&ix<=ntx-Lb-1)
	{
		...
		for(ic=0;ic<Lpo;ic++)
			dsigmaxx_dx+=Gp[index1*Lp+ic]*(s_sigmaxx[tyt][s_txp+ic+1]-s_sigmaxx[tyt][s_txp-ic])*one_over_dx;
		if((tx==0&&(ty==0||ty==BLOCK_SIZE-1))||(tx==BLOCK_SIZE-1&&(ty==0||ty==BLOCK_SIZE-1)))
			dsigmaxx_dx+=Gp[index1*Lp+Lpo]*(sigmaxx[ip+ntx+1]-sigmaxx[ip+ntx]+sigmaxx[ip-ntx+1]-sigmaxx[ip-ntx])*one_over_dx;	//x direction
		else 
			dsigmaxx_dx+=Gp[index1*Lp+Lpo]*(s_sigmaxx[tyt+1][s_txp+1]-s_sigmaxx[tyt+1][s_txp+1]+s_sigmaxx[tyt-1][s_txp+1]-s_sigmaxx[tyt-1][s_txp+1])*one_over_dx;

		for(ic=0;ic<Lso;ic++)
		{
			dsigmaxxs_dx+=Gs[index2*Ls+ic]*(s_sigmaxxs[tyt][s_txs+ic+1]-s_sigmaxxs[tyt][s_txs-ic])*one_over_dx;
			dsigmaxz_dz+=Gs[index2*Ls+ic]*(s_sigmaxz[s_tys+ic][txt]-s_sigmaxz[s_tys-(ic+1)][txt])*one_over_dz;
		}
		...
		if((tx==0&&(ty==0||ty==BLOCK_SIZE-1))||(tx==BLOCK_SIZE-1&&(ty==0||ty==BLOCK_SIZE-1)))
		{
			...
			dsigmaxxs_dx+=Gs[index2*Ls+Lso]*(s_sigmaxxs[tyt+1][s_txs+1]-s_sigmaxxs[tyt+1][s_txs]+s_sigmaxxs[tyt-1][s_txs+1]-s_sigmaxxs[tyt-1][s_txs])*one_over_dx;
			dsigmaxz_dz+=Gs[index2*Ls+Lso]*(s_sigmaxz[s_tys][txt+1]-s_sigmaxz[s_tys-1][txt+1]+s_sigmaxz[s_tys][txt-1]-s_sigmaxz[s_tys-1][txt-1])*one_over_dz;
		}
...
__global__ void fdtd_cpml_2d_GPU_kernel_vz_4LT_shared(
		float *rho, int itmax,float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *a_x, float *a_z_half,
		...
		)
{
	...
	/***start the assignment of the shared memory***/
	__shared__ float s_sigmaxx[BLOCK_SIZE+2*Lpo][BLOCK_SIZE];
	__shared__ float s_sigmaxz[BLOCK_SIZE][BLOCK_SIZE+2*Lso];
	__shared__ float s_sigmazz[BLOCK_SIZE+2*Lso][BLOCK_SIZE];
		
	s_sigmaxx[s_typ][tx]=sigmaxx[ip];
	s_sigmazz[s_tys][tx]=sigmazz[ip];
	s_sigmaxz[ty][s_txs]=sigmaxz[ip];	
		
	// in x_index_sigmaxz LsO
	if(tx<Lso)
		if(bx)
			s_sigmaxz[ty][tx]=sigmaxz[ip-Lso];
		else
			s_sigmaxz[ty][tx]=0.0;
	if(tx>blockDim.x-Lso-1)
		if(bx<gridDim.x-1)
			s_sigmaxz[ty][tx+2*Lso]=sigmaxz[ip+Lso];
		else
			s_sigmaxz[ty][tx+2*Lso]=0.0;

	// in y_index_sigmaxx LpO		
	if(ty<Lpo)
		if(by)
			s_sigmaxx[ty][tx]=sigmaxx[ip-Lpo*ntx];
		else
			s_sigmaxx[ty][tx]=0.0;
	if(ty>blockDim.y-Lpo-1)
		if(by<gridDim.y-1)
			s_sigmaxx[ty+2*Lpo][tx]=sigmaxx[ip+Lpo*ntx];
		else
			s_sigmaxx[ty+2*Lpo][tx]=0.0;

	// in y_index_sigmazz LSO		
	if(ty<Lso)
		if(by)
			s_sigmazz[ty][tx]=sigmazz[ip-Lso*ntx];
		else
			s_sigmazz[ty][tx]=0.0;
	if(ty>blockDim.y-Lso-1)
		if(by<gridDim.y-1)
			s_sigmazz[ty+2*Lso][tx]=sigmazz[ip+Lso*ntx];
		else
			s_sigmazz[ty+2*Lso][tx]=0.0;
	...
	if(iz>=Lb-1&&iz<=ntz-Lb-1&&ix>=Lb&&ix<=ntx-Lb)
	{
		...
		for(ic=0;ic<Lpo;ic++)
		{
			dsigmaxx_dz+=Gp[index1*Lp+ic]*(s_sigmaxx[s_typ+(ic+1)][tx]-s_sigmaxx[s_typ-ic][tx])*one_over_dz;
		}
		if(tx>0&&tx<BLOCK_SIZE-1&&ty>0&&ty<BLOCK_SIZE-1)
			dsigmaxx_dz+=Gp[index1*Lp+Lpo]*(s_sigmaxx[s_typ+1][tx+1]-s_sigmaxx[s_typ][tx+1]+s_sigmaxx[s_typ+1][tx-1]-s_sigmaxx[s_typ][tx-1])*one_over_dz;
		else
			dsigmaxx_dz+=Gp[index1*Lp+Lpo]*(sigmaxx[ip+ntx+1]-sigmaxx[ip+1]+sigmaxx[ip+ntx-1]-sigmaxx[ip-1])*one_over_dz;
		...
		for(ic=0;ic<Lso;ic++)
		{
			dsigmaxz_dx+=Gs[index2*Ls+ic]*(s_sigmaxz[ty][s_txs+ic]-s_sigmaxz[ty][s_txs-(ic+1)])*one_over_dx;
			dsigmazz_dz+=Gs[index2*Ls+ic]*(s_sigmazz[s_tys+(ic+1)][tx]-s_sigmazz[s_tys-ic][tx])*one_over_dz;
		}
		if(tx>0&&tx<BLOCK_SIZE-1&&ty>0&&ty<BLOCK_SIZE-1)
		{
			dsigmaxz_dx+=Gs[index2*Ls+Lso]*(s_sigmaxz[ty+1][s_txs]-s_sigmaxz[ty+1][s_txs-1]+s_sigmaxz[ty-1][s_txs]-s_sigmaxz[ty-1][s_txs-1])*one_over_dx;
			dsigmazz_dz+=Gs[index2*Ls+Lso]*(s_sigmazz[s_tys+1][tx+1]-s_sigmazz[s_tys][tx+1]+s_sigmazz[s_tys+1][tx-1]-s_sigmazz[s_tys][tx-1])*one_over_dz;
		}
		else{
			dsigmaxz_dx+=Gs[index2*Ls+Lso]*(sigmaxz[ip+ntx]-sigmaxz[ip+ntx-1]+sigmaxz[ip-ntx]-sigmaxz[ip-ntx-1])*one_over_dx;
			dsigmazz_dz+=Gs[index2*Ls+Lso]*(sigmazz[ip+ntx+1]-sigmazz[ip+1]+sigmazz[ip+ntx-1]-sigmazz[ip-1])*one_over_dz;
...
__global__ void fdtd_cpml_2d_GPU_kernel_sigmaxx_sigmazz_4LT_shared(
		float *rick, float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *lambda, float *lambda_plus_two_mu,
...

{
	...
	/***start the assignment of the shared memory***/
	__shared__ float s_vxp[BLOCK_SIZE][BLOCK_SIZE+2*Lpo];
	__shared__ float s_vxs[BLOCK_SIZE][BLOCK_SIZE+2*Lso];
	__shared__ float s_vzp[BLOCK_SIZE+2*Lpo][BLOCK_SIZE];
	__shared__ float s_vzs[BLOCK_SIZE+2*Lso][BLOCK_SIZE];
		
	s_vxp[ty][s_txp]=vx[ip];
	s_vxs[ty][s_txs]=vx[ip];
	s_vzp[s_typ][tx]=vz[ip];
	s_vzs[s_tys][tx]=vz[ip];	
		
	// in x_index_vxp LsO
	if(tx<Lpo)
		if(bx)
			s_vxp[ty][tx]=vx[ip-Lpo];
		else
			s_vxp[ty][tx]=0.0;
	if(tx>blockDim.x-Lpo-1)
		if(bx<gridDim.x-1)
			s_vxp[ty][tx+2*Lpo]=vx[ip+Lpo];
		else
			s_vxp[ty][tx+2*Lpo]=0.0;
	// in y_index_vzp LpO		
	if(ty<Lpo)
		if(by)
			s_vzp[ty][tx]=vz[ip-Lpo*ntx];
		else
			s_vzp[ty][tx]=0.0;
	if(ty>blockDim.y-Lpo-1)
		if(by<gridDim.y-1)
			s_vzp[ty+2*Lpo][tx]=vz[ip+Lpo*ntx];
		else
			s_vzp[ty+2*Lpo][tx]=0.0;

	// in x_index_vxs LsO
	if(tx<Lso)
		if(bx)
			s_vxs[ty][tx]=vx[ip-Lso];
		else
			s_vxs[ty][tx]=0.0;
	if(tx>blockDim.x-Lso-1)
		if(bx<gridDim.x-1)
			s_vxs[ty][tx+2*Lso]=vx[ip+Lso];
		else
			s_vxs[ty][tx+2*Lso]=0.0;
	// in y_index_vzs Lso		
	if(ty<Lso)
		if(by)
			s_vzs[ty][tx]=vz[ip-Lso*ntx];
		else
			s_vzs[ty][tx]=0.0;
	if(ty>blockDim.y-Lso-1)
		if(by<gridDim.y-1)
			s_vzs[ty+2*Lso][tx]=vz[ip+Lso*ntx];
		else
			s_vzs[ty+2*Lso][tx]=0.0;
	...
	if(iz>=Lb&&iz<=ntz-Lb&&ix>=Lb&&ix<=ntx-Lb)
	{
		...
		for(ic=0;ic<Lpo;ic++)
		{
			dvx_dx+=Gp[index1*Lp+ic]*(s_vxp[ty][s_txp+ic]-s_vxp[ty][s_txp-(ic+1)])*one_over_dx;
			dvz_dz+=Gp[index1*Lp+ic]*(s_vzp[s_typ+ic][tx]-s_vzp[s_typ-(ic+1)][tx])*one_over_dz;
		}
		if(tx>0&&tx<BLOCK_SIZE-1&&ty>0&&ty<BLOCK_SIZE-1)
		{
			dvx_dx+=Gp[index1*Lp+Lpo]*(s_vxp[ty+1][s_txp]-s_vxp[ty+1][s_txp-1]+s_vxp[ty-1][s_txp]-s_vxp[ty-1][s_txp-1])*one_over_dx;
			dvz_dz+=Gp[index1*Lp+Lpo]*(s_vzp[s_typ][tx+1]-s_vzp[s_typ-1][tx+1]+s_vzp[s_typ][tx-1]-s_vzp[s_typ-1][tx-1])*one_over_dz;
		}
		else{
			dvx_dx+=Gp[index1*Lp+Lpo]*(vx[ip+ntx]-vx[ip+ntx-1]+vx[ip-ntx]-vx[ip-ntx-1])*one_over_dx;
			dvz_dz+=Gp[index1*Lp+Lpo]*(vz[ip+1]-vz[ip-ntx+1]+vz[ip-1]-vz[ip-ntx-1])*one_over_dz;
		...
		for(ic=0;ic<Lso;ic++)
		{
			dvx_dx+=Gs[index2*Ls+ic]*(s_vxs[ty][s_txs+ic]-s_vxs[ty][s_txs-(ic+1)])*one_over_dx;
			dvz_dz+=Gs[index2*Ls+ic]*(s_vzs[s_tys+ic][tx]-s_vzs[s_tys-(ic+1)][tx])*one_over_dz;
		}
		if(tx>0&&tx<BLOCK_SIZE-1&&ty>0&&ty<BLOCK_SIZE-1)
		{
			dvx_dx+=Gs[index2*Ls+Lso]*(s_vxs[ty+1][s_txs]-s_vxs[ty+1][s_txs-1]+s_vxs[ty-1][s_txs]-s_vxs[ty-1][s_txs-1])*one_over_dx;
			dvz_dz+=Gs[index2*Ls+Lso]*(s_vzs[s_tys][tx+1]-s_vzs[s_tys-1][tx+1]+s_vzs[s_tys][tx-1]-s_vzs[s_tys-1][tx-1])*one_over_dz;
		}
		else{
			dvx_dx+=Gs[index2*Ls+Lso]*(vx[ip+ntx]-vx[ip+ntx-1]+vx[ip-ntx]-vx[ip-ntx-1])*one_over_dx;
			dvz_dz+=Gs[index2*Ls+Lso]*(vz[ip+1]-vz[ip-ntx+1]+vz[ip-1]-vz[ip-ntx-1])*one_over_dz;
...
__global__ void fdtd_cpml_2d_GPU_kernel_sigmaxz_4LT_shared(
		float *mu,float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *a_x_half, float *a_z_half,
		...
		)
{
	...
	/***start the assignment of the shared memory***/
	__shared__ float s_vz[BLOCK_SIZE][BLOCK_SIZE+2*Lso];
	__shared__ float s_vx[BLOCK_SIZE+2*Lso][BLOCK_SIZE];
	...
	if(tx<Lso)
		if(bx)
			s_vz[ty][tx]=vz[ip-Lso];
		else
			s_vz[ty][tx]=0.0;
	if(tx>blockDim.x-Lso-1)
		if(bx<gridDim.x-1)
			s_vz[ty][tx+2*Lso]=vz[ip+Lso];
		else
			s_vz[ty][tx+2*Lso]=0.0;
	// in y_index_vxs LsO		
	if(ty<Lso)
		if(by)
			s_vx[ty][tx]=vx[ip-Lso*ntx];
		else
			s_vx[ty][tx]=0.0;
	if(ty>blockDim.y-Lso-1)
		if(by<gridDim.y-1)
			s_vx[ty+2*Lso][tx]=vx[ip+Lso*ntx];
		else
			s_vx[ty+2*Lso][tx]=0.0;
	...
	if(iz>=Lb-1&&iz<=ntz-Lb-1&&ix>=Lb-1&&ix<=ntx-Lb-1)
	{
		...
		for(ic=0;ic<Lso;ic++)
		{
			dvz_dx+=Gs[index2*Ls+ic]*(s_vz[ty][s_txs+ic+1]-s_vz[ty][s_txs-ic])*one_over_dx;
			dvx_dz+=Gs[index2*Ls+ic]*(s_vx[s_tys+(ic+1)][tx]-s_vx[s_tys-ic][tx])*one_over_dz;
		}

		if(tx>0&&tx<BLOCK_SIZE-1&&ty>0&&ty<BLOCK_SIZE-1)
		{
			dvz_dx+=Gs[index2*Ls+Lso]*(s_vz[ty+1][s_txs+1]-s_vz[ty+1][s_txs]+s_vz[ty-1][s_txs+1]-s_vz[ty-1][s_txs])*one_over_dx;
			dvx_dz+=Gs[index2*Ls+Lso]*(s_vx[s_tys+1][tx+1]-s_vx[s_tys][tx+1]+s_vx[s_tys+1][tx-1]-s_vx[s_tys][tx-1])*one_over_dz;
		}
		else{
			dvz_dx+=Gs[index2*Ls+Lso]*(vz[ip+1+ntx]-vz[ip+ntx]+vz[ip+1-ntx]-vz[ip-ntx])*one_over_dx;
			dvx_dz+=Gs[index2*Ls+Lso]*(vx[ip+ntx+1]-vx[ip+1]+vx[ip+ntx-1]-vx[ip-1])*one_over_dz;
...
//			printf("it == %d\n",it);
		...
		for(i=0;i<GPU_N;i++)
		{
			...
			fdtd_cpml_2d_GPU_kernel_vx_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
			...
			fdtd_cpml_2d_GPU_kernel_vz_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rho, itmax,plan[i].d_Gp, plan[i].d_Gs,
				 plan[i].d_vp, vp_min, dvp, plan[i].d_vs, vs_min, dvs,
			...
			fdtd_cpml_2d_GPU_kernel_sigmaxx_sigmazz_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
			...
			fdtd_cpml_2d_GPU_kernel_sigmaxz_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
...
__global__ void fdtd_2d_GPU_kernel_sigmaxx_sigmazz_backward_4LT_shared(
		float *rick, float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *lambda, float *lambda_plus_two_mu,
...
__global__ void fdtd_2d_GPU_kernel_sigmaxz_backward_4LT_shared(
		float *mu, float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *vx, float *vz, float *sigmaxz,
...
__global__ void fdtd_2d_GPU_kernel_vx_backward_4LT_shared(
		float *rho,float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *vx, float *sigmaxx, float *sigmaxxs, float *sigmaxz,
...
__global__ void fdtd_2d_GPU_kernel_vz_backward_4LT_shared(
		float *rho,float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *vz, float *sigmaxz, float *sigmaxx,float *sigmazz,
...
__global__ void fdtd_cpml_2d_GPU_kernel_vx_backward_4LT_shared(
		float *rho,float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *a_x_half, float *a_z, 
...
__global__ void fdtd_cpml_2d_GPU_kernel_vz_backward_4LT_shared(
		float *rho,float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *a_x, float *a_z_half,
...
__global__ void fdtd_cpml_2d_GPU_kernel_sigmaxx_sigmazz_backward_4LT_shared(
		float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *lambda, float *lambda_plus_two_mu,
...
__global__ void fdtd_cpml_2d_GPU_kernel_sigmaxz_backward_4LT_shared(
		float *mu,float *Gp, float *Gs,
		float *vp, float vpmin, float dvp,
		float *vs, float vsmin, float dvs,
		float *a_x_half, float *a_z_half,
...
*/
	...
	for(it=itmax-2;it>=0;it--)
	{
		...
		for(i=0;i<GPU_N;i++)
		{
			...
			fdtd_2d_GPU_kernel_sigmaxx_sigmazz_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rick, plan[i].d_Gp, plan[i].d_Gs,
				 plan[i].d_vp, vp_min, dvp, plan[i].d_vs, vs_min, dvs,
				 plan[i].d_lambda, plan[i].d_lambda_plus_two_mu,
			...
			fdtd_2d_GPU_kernel_sigmaxz_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_mu,plan[i].d_Gp, plan[i].d_Gs,
				 plan[i].d_vp, vp_min, dvp, plan[i].d_vs, vs_min, dvs,
			...
			fdtd_2d_GPU_kernel_vx_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
			...
			fdtd_2d_GPU_kernel_vz_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rho,plan[i].d_Gp, plan[i].d_Gs,
				 plan[i].d_vp, vp_min, dvp, plan[i].d_vs, vs_min, dvs,
			...
			/////////////////////////receiver wavefield////////////////////////////
			///////////////////////////////////////////////////////////////////////
			fdtd_cpml_2d_GPU_kernel_vx_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rho, plan[i].d_Gp, plan[i].d_Gs,
				 plan[i].d_vp, vp_min, dvp, plan[i].d_vs, vs_min, dvs,
				 plan[i].d_a_x_half, plan[i].d_a_z, 
			...
			fdtd_cpml_2d_GPU_kernel_vz_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_rho, plan[i].d_Gp, plan[i].d_Gs,
				 plan[i].d_vp, vp_min, dvp, plan[i].d_vs, vs_min, dvs,
				 plan[i].d_a_x, plan[i].d_a_z_half,
			...
			fdtd_cpml_2d_GPU_kernel_sigmaxx_sigmazz_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
			...
			fdtd_cpml_2d_GPU_kernel_sigmaxz_backward_4LT_shared<<<dimGrid,dimBlock,0,plans[i].stream>>>
				(
				 plan[i].d_mu, plan[i].d_Gp, plan[i].d_Gs,
				 plan[i].d_vp, vp_min, dvp, plan[i].d_vs, vs_min, dvs,
				 plan[i].d_a_x_half, plan[i].d_a_z_half,
